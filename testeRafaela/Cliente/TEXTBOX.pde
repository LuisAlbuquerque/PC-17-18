public class TEXTBOX {
   ...
   boolean KEYPRESSED(char KEY, int KEYCODE) {
      if (selected) {
         if (KEYCODE == (int)BACKSPACE) {
            ...
            boolean isKeySimbolo = (KEY == '-' || KEY == '.' || KEY == '@' || KEY == '_');
      
            if (isKeyCapitalLetter || isKeySmallLetter || isKeyNumber || isKeySimbolo) {
               addText(KEY);
            }
         }
      }
   ...
   void PRESSED(int x, int y, TEXTBOX t) {
      if (overBox(x, y)) {
         selected = true;
         t.BorderEnable = true;
      } else {
